  ...
  def checked
    case (params[:type])
      when "productask"
        ...
        @redirect_path = useradmin_productask_path(@message.id)

      when "deal"
        @message = Deal.find(params[:id])
        @redirect_path = useradmin_deal_path(@message.id)
        
      when "dealask"
        @message = Dealask.find(params[:id])
        @redirect_path = useradmin_deal_path(@message.deal_id)
        
      when "dealvalue"
        @message = Dealvalue.find(params[:id])
        @redirect_path = dealvalues_useradmin_deal_path(@message.deal_id)
        
      when "myask"
        @message = Productaskre.find(params[:id])
        @redirect_path = useradmin_myask_path(@message.productask_id)
